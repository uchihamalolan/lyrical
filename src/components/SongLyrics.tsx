...
function LyricsError({ children }: { children: React.ReactNode }) {
	return (
...
export function Lyrics({ lyrics }: { lyrics: string }) {
	...
	const [transliteratedText, setTransliteratedText] = useState<string | null>(
		null,
	);
	...
	const displayText =
		isTransliterated && transliteratedText ? transliteratedText : lyrics;
	...
	return (
		<div className="bg-base-200/40 backdrop-blur-xl rounded-3xl p-8 sm:p-12 md:p-16 shadow-2xl border border-base-content/5 relative overflow-hidden before:absolute before:top-0 before:left-0 before:w-full before:h-1 before:bg-gradient-to-r before:from-transparent before:via-primary/20 before:to-transparent">
			<div className="absolute top-6 right-6 z-20 flex flex-col items-end gap-2">
				...
				{error && <LyricsError>{error}</LyricsError>}
			</div>

			<article
				className={`font-sans tracking-tight transition-all duration-700 ${isLoading ? "opacity-20 blur-md scale-[0.98]" : "opacity-100 scale-100"}`}
			>
				{lines.map((line, index) => (
					<p
						...
						className={
							line.trim() === ""
								? "h-12"
								: "text-base-content text-xl sm:text-2xl md:text-3xl leading-relaxed font-normal hover:text-primary transition-colors duration-200"
						}
					>
						{line.trim() === "" ? "\u00A0" : line}
					</p>
